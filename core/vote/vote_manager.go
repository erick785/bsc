			...
			for {
				...
				case newCHead := <-voteManager.highestVerifiedBlockCh:
					...
					if newCHead.Header.Number.Uint64() == selectedCHead.Header.Number.Uint64() {
						// Compare hash values, select the one with larger hash
						// if bytes.Compare(newCHead.Header.Hash().Bytes(), selectedCHead.Header.Hash().Bytes()) > 0 {
						...
						//}
						// if newCHead.Header.Difficulty.Int64() > selectedCHead.Header.Difficulty.Int64() {
						// 	selectedCHead = newCHead
