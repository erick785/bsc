...
// BakcupNodes 按照backOffTime排序从小到大排序
type BackupNodes []*BackupNode

func (b BackupNodes) Len() int {
	return len(b)
}

func (b BackupNodes) Less(i, j int) bool {
	return b[i].backOffTime < b[j].backOffTime
}

func (b BackupNodes) Swap(i, j int) {
	b[i], b[j] = b[j], b[i]
}
...
func (p *Parlia) GetBackupNode(chain consensus.ChainHeaderReader, header *types.Header) ([]*common.Address, error) {
	...
	nodes := make(BackupNodes, 0)

	for addr, _ := range validators {
		nodes = append(nodes, &BackupNode{
			Addr:        common.HexToAddress(addr),
			backOffTime: p.backOffTime(snap, header, common.HexToAddress(addr)),
		})
	}

	sort.Sort(nodes)

	backupNodes := make([]*common.Address, 0)
	for _, node := range nodes {
		log.Info("backup node", "number", header.Number, "addr", node.Addr, "node", validators[strings.ToLower(node.Addr.Hex())], "backOffTime", node.backOffTime)
		if node.backOffTime < 4 {
			backupNodes = append(backupNodes, &node.Addr)
		}
	}

	return backupNodes, nil
}
